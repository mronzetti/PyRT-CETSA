...
with open(os.path.join(__location__, "VERSION"), "r") as version_file:
        ...
        if input_dict.get("version") and input_dict.get("timestamp"):
            print(
                "Information: JSON session was created with MoltenProt v. {} on {}".format(
                    input_dict["version"], input_dict["timestamp"]
                )
            )
        else:
            print(
...
class MoltenProtFit:
    ...
    def __init__(
        ...
        if input_type == "from_dict":
            ...
            if denaturant == "K":
                ...
                raise ValueError("NO Fahrenheit, please!")
            else:
                # if temperature was not recognized, set denaturant to chemical type
                # NOTE currently not implemented in MoltenProtFit
                print("Assuming that {} is a chemical denaturant".format(denaturant))
                self.denaturant = denaturant
                raise NotImplementedError("Chemical denaturation not implemented yet")
    ...
    def heatmap(
        ...
        if (((plate96 < 0).any())).any():
            ...
            col_min = "{:.3f}".format(self.plate_results[use_column].min())
            col_max = "{:.3f}".format(self.plate_results[use_column].max())
            if lowerbetter:
                tick_labels = [col_max, col_min]
            else:
                tick_labels = [col_min, col_max]
    ...
    def print_message(self, text, message_type):
        ...
        if message_type == "f":
            ...
            msg = "Warning: " + text + " ({})".format(self.readout_type)
            print(msg)
            msg = msg + "\n"
            self.protocolString = self.protocolString + msg
        elif message_type == "i":
            msg = "Information: " + text + " ({})".format(self.readout_type)
            print(msg)
            msg = msg + "\n"
            self.protocolString = self.protocolString + msg
        else:
    ...
    def _calculate_raw_corr(self):
        ...
        def calculate_raw_corr_series(input_series):
            ...
            if input_series.name in self.plate_results.index:
                ...
                output = pd.Series(
                    output, index=self.plate.index, name=input_series.name
                )
                self.plate_raw_corr = pd.concat(
                    [self.plate_raw_corr, pd.Series(output)], axis=1
                )
        ...
        if estimate_Tm:
            ...
            if (dintersect > tmin + fit_length) and (dintersect < tmax - fit_length):
                ...
                if b_diff > 0:
                    ...
                    self.print_message(
                        "Using the middle of temperature range as Tm_init", "i"
                    )
                    self.plate_results.loc["Tm_init", input_series.name] = tmid
    ...
    def plotfig(
        ...
        data_ax.set_xlabel("Temperature, {}".format(degree_sign))
        ...
        if datatype == "overview":
            ...
            for parameter_name in self.plotlines:
                if vline_legend:
                    ...
                    data_ax.axvline(
                        self.plate_results[parameter_name][i], ls="dotted", c="b", lw=3
                    )
            ...
            if deriv_ax is not None:
                ...
                deriv_ax.plot(
                    self.plate[i].index.values, self.plate_derivative[i], color="k"
                )
                deriv_ax.set_xlabel("Temperature, {}".format(degree_sign))
                deriv_ax.set_ylabel("d({})/dT".format(self.readout_type))
    ...
    def html_heatmap(self, heatmap_cmap, display):
            ...
            for j in range(1, 13):
                ...
                if sample_id in colors.index:
                    line = line + sample.substitute(
                        ID=sample_id, COLOR=colors[sample_id], CONDITION=CONDITION
                    )
                else:
                    # lightgray blends with mid-range of coolwarm, so change to gray
                    line = line + sample.substitute(
                        ID=sample_id, COLOR="gray", CONDITION=CONDITION
                    )
        ...
        title_text = "{}: heatmap of {} (model <i>{}</i>)".format(
            self.readout_type, self.plate_results.iloc[:, -1].name, self.model
        )
        output = output_template.substitute(
            DISPLAY=display,
    ...
    def html_button(self):
        ...
        button_template = button_template.substitute(IDENTIFIER=self.readout_type)
    ...
    def printAnalysisSettings(self):
        ...
        for setting, def_value in analysis_defaults.items():
            if setting[-2:] == "_h":
                ...
                print(
                    "{} = {} (default: {})".format(
                        setting, getattr(self, setting), def_value
                    )
                )
        print("\n")
    ...
    def SetLayout(
        ...
        if dCp >= 0:
            ...
            self.print_message("dCp for all samples is set to {}".format(dCp), "i")
            self.dCp = dCp
            ...
            raise ValueError("Incorrect value for dCp ({})!".format(dCp))
    ...
    def PrepareData(self):
        ...
        if self.exclude:
            self.plate = self.plate.drop(
                self.plate.columns.intersection(self.exclude), axis=1
            )
        ...
        if self.blanks:
            ...
            try:
                ...
                self.plate = self.plate.drop(
                    self.plate.columns.intersection(self.blanks), axis=1
                )
        ...
        if self.mfilt:
            ...
            if len(self.plate.index) < self.mfilt:
                ...
                raise ValueError(
                    "Specified medfilt window size ({} datapoints) is bigger than the dataset length ({} datapoints)!".format(
                        self.mfilt, len(self.plate.index)
                    )
                )
        ...
        if self.shrink is not None:
            if self.shrink > self.dT:
                ...
                self.plate_binned = pd.DataFrame(
                    index=bin_range[:-1], columns=self.plate.columns, dtype="float64"
                )
                ...
                self.print_message(
                    "Input data binned down to {} degree step".format(self.shrink), "i"
                )
                # required for proper derivative computation
                self.dT = self.shrink
            else:
                raise ValueError(
                    "Requested shrinking step ({} degrees) is less than the average temperature step ({} degrees)".format(
                        self.shrink, self.dT
                    )
                )
        ...
        try:
            ...
            if len(self.plate.index) < window_length:
                # NOTE the output may be confusing to the user, because user gives degrees, but gets datapoints
                raise ValueError(
                    "Specified savgol window size ({} datapoints) is bigger than the dataset length ({} datapoints)!".format(
                        window_length, len(self.plate.index)
                    )
                )
            ...
            if window_length < 4:
                raise ValueError(
                    "Specified savgol window size ({} datapoints) is smaller than the polynomial order of the filter (4); increase savgol window size, or do not shrink the data".format(
                        window_length
                    )
                )
            """
    ...
    def ProcessData(self):
        ...
        for i in df_for_fitting.columns.values:
            ...
            if set(["kN", "bN", "kU", "bU"]).issubset(model.param_names()):
                ...
                if self.baseline_bounds > 0:
                    ...
                    raise ValueError(
                        "Expected a non-negative int for baseline_bounds, but got {}".format(
            ...
            try:
                ...
                self.print_message(
                    "Curve fit for {} failed, probably invalid transition.".format(i),
                    "w",
                )
        ...
        if set(["kN", "bN", "kU", "bU", "Tm"]).issubset(model.param_names()):
        ...
        if self.layout is not None:
            self.plate_results = pd.concat(
                [self.layout, self.plate_results], join="inner", axis=1
            )

        # remove *_init columns and S from self.plate_results_stdev
        self.plate_results_stdev.drop(
            labels=result_index[: len(p0)], axis=1, inplace=True
        )
        ...
        if model.sortby == "dG_std":
            ...
            self.plate_results["pk_ratio_std"] = -np.log10(
                model.arrhenius(
                    T_std, self.plate_results.TfF_fit, self.plate_results.EaF_fit
                )
                / model.arrhenius(
                    T_std, self.plate_results.TfR_fit, self.plate_results.EaR_fit
                )
            )
            ...
            self.plate_results["pk_std"] = -np.log10(
                model.arrhenius(
                    T_std, self.plate_results.Tf_fit, self.plate_results.Ea_fit
                )
            )
        ...
        if model.sortby is not None:
            self.plate_results.sort_values(
                by=model.sortby, inplace=True, ascending=False
            )
    ...
    def CalculateThermodynamic(self):
        ...
        td_results = pd.DataFrame(
            index=self.plate_results.index, columns=column_names, dtype="float64"
        )
        # create a separate dataframe for storing stdev
        td_results_stdev = pd.DataFrame(
            index=self.plate_results.index, columns=[column_names[1]], dtype="float64"
        )
        ...
        self.plate_results_stdev = pd.concat(
            [self.plate_results_stdev, td_results_stdev], axis=1, sort=True
        )
    ...
    def RenameResults(self, datatype="sct"):
        ...
        if datatype == "sct":
            ...
            raise NotImplementedError("Chemical denaturation renaming is not there yet")
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.heatmap(
                                output_path, plate96, i, title="Heatmap of " + str(i)
                            )
                        except:
                            self.print_message(
                                "Re-check the parameters for --heatmap option", "w"
                            )
                except ValueError:
                    # NOTE a temporary fix for cases when a wrong heatmap parameter was supplied
                    self.print_message(
                        '"{}" is an invalid option for heatmap'.format(i), "w"
                    )
                ...
                if len(failed_samples) > 0:
                    Parallel(n_jobs=n_jobs)(
                        delayed(self.plotfig)(
                            output_path=output_path, datatype="raw", wellID=i
                        )
...
class MoltenProtFitMultiple:
    ...
    def GetAnalysisSettings(self):
        ...
        model_settings = pd.Series(
            index=self.GetDatasets(), name="Model", data="santoro1988"
        )
    ...
    def AddDataset(self, data, readout):
        ...
        if readout in list(self.datasets.keys()):
            raise RuntimeError(
                "Readout '{}' is already present in this MoltenProtFitMultiple instance!"
            )
    ...
    def RenameResultsColumns(self, which, mapping):
        ...
        self.print_message(
            "Renaming results columns in dataset {}\n{}".format(which, mapping), "i"
        )
        # print(mapping)
        ...
        if parallelization and n_jobs > 1:
            if len(self.GetDatasets()) == 1:
                ...
                self.WriteOutputSingle(
                    self.GetDatasets()[0], outfolder, n_jobs=n_jobs, **output_kwargs
                )
            else:
                Parallel(n_jobs=n_jobs)(
...
class MoltenProtFitMultipleLE(MoltenProtFitMultiple):
    ...
    def PrepareAndAnalyseAll(self, n_jobs=1):
        ...
        if not "Scattering" in self.GetDatasets():
            raise ValueError("lumry_eyring model requires a Scattering dataset")
...
def parse_prom_xlsx(filename, raw=False, refold=False, LE=False, deltaF=True):
    ...
    * the sheets have a standardized order in the file: Overview, Readout1_(unfolding), Readout1_(unfolding)_derivative ...,  Readout1_(refolding), Readout1_(refolding)_derivative ... ; this can be used to parse data independently of sheet labels; also, the sheets containing "deriv" can be auto-excluded
    ...
    if not "Overview" in input_xlsx:
        raise ValueError("Input file {} contains no overview sheet".format(filename))
    ...
    if deltaF:
        ...
        elif set(["330nm_(Unfolding)", "350nm_(Unfolding)"]).issubset(
            output.GetDatasets()
        ):
